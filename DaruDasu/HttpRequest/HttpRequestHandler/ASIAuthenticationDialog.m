...
- (UITextField *)textFieldInRow:(NSUInteger)row section:(NSUInteger)section
{
	return [[[[[self tableView] cellForRowAtIndexPath:
			  ...
			  contentView] subviews] safeObjectAtIndex:0];
}
...
- (void)showTitle
{
	UINavigationBar *navigationBar = [[[self view] subviews] safeObjectAtIndex:0];
	UINavigationItem *navItem = [[navigationBar items] safeObjectAtIndex:0];
...
- (void)show
{
	...
	[[[[[self tableView] cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]].contentView subviews] safeObjectAtIndex:0] becomeFirstResponder];
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		ASIHTTPRequest *nextRequest = [requestsNeedingAuthentication safeObjectAtIndex:0];
		[requestsNeedingAuthentication removesafeObjectAtIndex:0];
